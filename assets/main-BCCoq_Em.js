    ...
    quantityInputs.forEach((element) => {
      ...
      if (name.includes("cart-")) {
        ...
        if (keyMatch) {
          ...
          const cartItem = this.cart.items.find((item) => item.key.toString() === key);
...
const products = {
  ...
  handleProductFormChange(initialVariant, sectionId, productHandle, optionPosition, optionValueId, sellingPlanGroupId = 0, sellingPlanId = 0, enableDefault = true) {
    ...
    const optionsMap = Array.from(
      ((_a = document.querySelector(`.options-${sectionId}`)) == null ? void 0 : _a.querySelectorAll("[data-option-value-id].js-activeVariant")) || []
    ).reduce((map, el) => {
      const htmlEl = el;
    ...
    if (!enableDefault) {
      ...
      this.fetchAndUpdateOptions(sectionId, productHandle, void 0, void 0, this.current_variant, enableDefault, sellingPlanGroupId, sellingPlanId);
    } else {
      this.fetchAndUpdateOptions(sectionId, productHandle, optionPosition, optionValueId, void 0, enableDefault, sellingPlanGroupId, sellingPlanId);
    }
  },
  fetchAndUpdateOptions(sectionId, productHandle, optionPosition, optionValueId, initialVariant, enableDefault = true, sellingPlanGroupId = 0, sellingPlanId = 0) {
    ...
    fetch(
      ...
      this.current_variant = (variantJson == null ? void 0 : variantJson.textContent) || "";
      let selectedVariant = null;
      if (this.current_variant && this.current_variant.trim() !== "") {
        try {
          selectedVariant = JSON.parse(this.current_variant);
        } catch (error2) {
          console.warn("Failed to parse current_variant JSON:", this.current_variant, error2);
      ...
      if (newOptionsElement && (optionsElement == null ? void 0 : optionsElement.parentNode)) {
  ...
  setallOptionsSelected() {
    let optionsSize = this.product.options.length;
    this.all_options_selected = optionsSize === 1 && this.option1 || optionsSize === 2 && this.option1 && this.option2 || optionsSize === 3 && this.option1 && this.option2 && this.option3 || optionsSize === 4 && this.option1 && this.option2 && this.option3 && this.option4;
...
const collections = {
  ...
  async fetchAndRenderTableRows(product_handle, total_pages, current_page, section_id, inject_before) {
...
function createIterableMethod(method, isReadonly2, isShallow2) {
  return function(...args) {
    ...
    const wrap = isReadonly2 ? toReadonly : toReactive;
    !isReadonly2 && track(
      rawTarget,
      "iterate",
...
function createInstrumentations(readonly2, shallow) {
  const instrumentations = {
    get(key) {
      ...
      const wrap = readonly2 ? toReadonly : toReactive;
  ...
  extend(
    ...
    readonly2 ? {
      ...
      add(value) {
        ...
        const valueToAdd = !isShallow(value) && !isReadonly(value) ? rawValue : value;
  ...
  iteratorMethods.forEach((method) => {
    instrumentations[method] = createIterableMethod(method, readonly2);
  });
  return instrumentations;
}
function createInstrumentationGetter(isReadonly2, shallow) {
  const instrumentations = createInstrumentations(isReadonly2);
...
function src_default(Alpine2) {
  let persist = () => {
    ...
    return Alpine2.interceptor((initialValue, getter, setter, path, key, cleanup = () => {
    }) => {
      let lookup = alias || `_x_${path}`;
      let initial = storageHas(lookup, storage) ? storageGet(lookup, storage) : initialValue;
      setter(initial);
      let effect3 = Alpine2.effect(() => {
        let value = getter();
        storageSet(lookup, value, storage);
        setter(value);
      });
      cleanup(() => Alpine2.release(effect3));
      return initial;
    }, (func) => {
      func.as = (key) => {
        alias = key;
  ...
  Alpine2.persist = (key, { get: get2, set: set2 }, storage = localStorage) => {
